...
void Function::gradient(gsl_vector *x, gsl_vector *grad){
    ...
    double fx = f1(x);
    for(int i = 0; i<n; i++){
        gsl_vector_set_basis(_x_grad, i);
        gsl_vector_scale(_x_grad, dx);
        gsl_blas_daxpy(1., x, _x_grad);
...
void Function::jacobian(gsl_matrix *J, gsl_vector *x, gsl_vector *fx){
    ...
    for(int k = 0; k<n; k++){
        gsl_vector_set_basis(_x_jaco, k);
        gsl_vector_scale(_x_jaco, dx);
        gsl_blas_daxpy(1., x, _x_jaco);
        f2(_x_jaco, _f);
        for(int i = 0; i<n; i++){
