	...
	do{
		...
		for(int p=0;p<n-1;p++){
            for(int q=p+1;q<n;q++){
                ...
                double old_app=gsl_matrix_get(A,p,p);
                double old_aqq=gsl_matrix_get(A,q,q);
                ...
                double givens_angle=atan2(apq-aqp, app+aqq);            //Angle for Givens matrix
                ...
                //Then eliminate them
                double jacobi_angle=0.5*atan2(2*apq,aqq-app);            //Angle for Jacobi matrix
                ...
                double new_app=gsl_matrix_get(A,p,p);
                double new_aqq=gsl_matrix_get(A,q,q);
